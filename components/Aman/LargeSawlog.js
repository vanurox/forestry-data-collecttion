...
export default class Pole extends Component {
  ...
  nextScreen=()=>{
    AsyncStorage.setItem(`large_sawlog_ugs${this.props.navigation.state.params.id}`, this.state.ugsValue.toString());
    AsyncStorage.setItem(`large_sawlog_ags${this.props.navigation.state.params.id}`, this.state.agsValue.toString());
      ...
      <View style={styles.container}>
        ...
        >
          <View style={{ width: "100%",  height: 30, justifyContent: 'flex-start', marginTop: 40,backgroundColor:'rgba(0,0,0,0.5)' }}>
            <Text style={{ alignSelf: 'center', color: "#fff", fontSize: 25, fontWeight: 'bold' }}>Large Sawlog</Text>
          </View>
